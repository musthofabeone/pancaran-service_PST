...
async function Mobopay() {
  ...
  let result = await executeQuery('SELECT T0.* FROM (SELECT ROW_NUMBER () OVER ( ' +
    ...
    'GROUP BY X0."REFERENCY", X0."TRANSFERTYPE",X0."TRANSDATE", X0."TRANSTIME", X0."DB", X0."ACTION", X0."PAYMENTOUTTYPE", X1."TRXID" ' +
...
async function getConfigDB() {
  let _dbName = "";
  await fs.readFile('configDB.txt', 'utf8', (err, data) => {
    if (err) {
      console.error(err);
      return;
    }else{
...
let jobs = cron.schedule('*/10 * * * * *', async () => {
  const _dbName = await getConfigDB();
  await jobs.stop()
  logger.info('Service Create Outgoing Payment.............................');
  ...
  try {
      ...
      let result = await executeQuery('SELECT DISTINCT X0."trxId", X0."customerReffNumber", CAST(X1."AMOUNT" AS DECIMAL(19,2)) "AMOUNT", CAST(X1."BANKCHARGE" AS DECIMAL(19,2)) "BANKCHARGE", X1."DB", X0."auth", X1."PAYMENTOUTTYPE"  FROM paymenth2h."BOS_DO_STATUS" X0 ' +
          'INNER JOIN paymenth2h."BOS_TRANSACTIONS" X1 ON X0."trxId" = X1."TRXID" AND X0."customerReffNumber" = X1."REFERENCY" ' +
          'WHERE X0."flag" = \'0\' AND X1."DB" = $1 ORDER BY X0."customerReffNumber" ASC', [_dbName])
      ...
      if (result.rowCount > 0) {
          for await (var data of result.rows) {
              ...
              post_Incoming(data.trxId, data.customerReffNumber, data.AMOUNT, data.auth, data.DB)
              //await delay(30000);
          }
      }
...
function notConfirmedInterval(reference, db, outType, trxId) {
  var start = new Date();
  // insert ke log jika terkena cutoff
  executeQuery('INSERT INTO paymenth2h."BOS_LOG_TRANSACTIONS"("PAYMENTOUTTYPE", "PAYMENTNO", "TRXID", "REFERENCY", "VENDOR", "ACCOUNT", "AMOUNT", "TRANSDATE", "TRANSTIME", "STATUS", "REASON", "BANKCHARGE", "FLAGUDO", "SOURCEACCOUNT", "TRANSFERTYPE", "CLIENTID", "ERRORCODE")' +
  ...
  executeQuery('UPDATE paymenth2h."BOS_TRANSACTIONS" SET "INTERFACING" = \'1\' WHERE "REFERENCY" = $1', [reference], function (error1, result, fields) {
    if (error1) {
      logger.error("Update flag interfacing error (" + db + "), Ref: " + reference + ", msg: " + error1)
    } else {
      logger.info("Update flag interfacing Success (" + db + "), Ref: " + reference)
      getEntryUdoOut(referency, db, trxId, outType, true)
    }
  });
}
...
async function Login_V2(dbName) {
  ...
  await fetchWithRetry(config.base_url_SL + "/b1s/v2/Login", requestOptions, 120000, 3)
  ...
  await fetch(config.base_url_xsjs + "/PaymentService/get_Header.xsjs?trxid=" + trxid + "&dbName=" + dbName, {
    ...
    .then(async data => {
      ...
      if (data != null) {
        ...
        await fetch(config.base_url_SL + '/b1s/v2/VendorPayments', {
          method: 'POST',
                  ...
                  .then(async data => {
                    Object.keys(data.LINES).forEach(async function (key) {
                      ...
                      if (d.Type == "OUT") // jika payment out
                      {
                        ...
                        // update UDO
                        await fetch(config.base_url_SL + "/b1s/v2/PYM_OUT(" + parseFloat(d.DocEntry) + ")",
                          {
                            method: 'PATCH',
                            headers: {
                              'Content-Type': 'application/json'
                      ...
                      } else if (d.Type == "OUTSTATUS") // jika payment out STATUS
                      {
                        ...
                        // update UDO
                        await fetch(config.base_url_SL + "/b1s/v2/PYMOUT_STATUS(" + parseFloat(d.DocEntry) + ")",
                          {
                            method: 'PATCH',
                            headers: {
                              'Content-Type': 'application/json'
            ...
            if (cekError.rowCount > 0) {
              for (var data of cekError.rows) {
                ...
                if (parseFloat(data.RESENDCOUNT) <= config.resend_payment_retries) {
                  ...
                  await executeQuery('UPDATE  paymenth2h."BOS_TRANSACTIONS_D" SET "chargingModelId" = $1  WHERE "customerReferenceNumber" = $2', [req.body.chargingModelId, req.body.referency])
  ...
  connection.query('select * from paymenth2h."BOS_DB_LIST" WHERE "NM_DB" = $1', [req.body.dbName], function (error, result, fields) {
    if (error) {
      ...
      for (var data of result.rows) {
        ...
        var fs = require('fs');
        fs.writeFile('configDB.txt', dbName, function (err) {
          if (err) throw err;
          console.log('Saved!');
        });
      }
    ...
    while (count > 0) {
      try {
        ...
        if (count > 0) {
          ...
          updateTableRetry(referency, "4", "Resend Payment")
          await getEntryUdoOut(referency, dbName, trxId, outType, false) // jika mencapai maksimum retry, maka update detail UDO menjadi failed
        }
      }
    }
  }
  ...
  await fetchWithRetry(config.base_url_payment + "/mobopay/mandiri/payment/do-payment", requestOptions, config.timeOut, config.max_retries)
    ...
    .then(async result => {
      ...
      if (resultBody.resultCode == "0") {
        ...
        executeQuery('UPDATE paymenth2h."BOS_TRANSACTIONS" SET "INTERFACING" = \'1\', "SUCCESS" = \'N\' WHERE "REFERENCY" = $1', [referency])


        getEntryUdoOut(referency, dbName, trxId, outType, true)
        executeQuery('INSERT INTO paymenth2h."BOS_LOG_TRANSACTIONS"("PAYMENTOUTTYPE", "PAYMENTNO", "TRXID", "REFERENCY", "VENDOR", "ACCOUNT", "AMOUNT", "TRANSDATE", "TRANSTIME", "STATUS", "REASON", "BANKCHARGE", "FLAGUDO", "SOURCEACCOUNT", "TRANSFERTYPE", "CLIENTID", "ERRORCODE")' +
          'SELECT "PAYMENTOUTTYPE", "PAYMENTNO","TRXID", "REFERENCY", "VENDOR", "ACCOUNT", "AMOUNT", $3, $4, 4, $1, "BANKCHARGE", "FLAGUDO", "SOURCEACCOUNT", "TRANSFERTYPE", "CLIENTID",  $5' +
        ...
        executeQuery('INSERT INTO paymenth2h."BOS_LOG_TRANSACTIONS"("PAYMENTOUTTYPE", "PAYMENTNO", "TRXID", "REFERENCY", "VENDOR", "ACCOUNT", "AMOUNT", "TRANSDATE", "TRANSTIME", "STATUS", "REASON", "BANKCHARGE", "FLAGUDO", "SOURCEACCOUNT", "TRANSFERTYPE", "CLIENTID")' +
        ...
        getEntryUdoOut(referency, dbName, resultBody.data.trxId, outType, false)
        connection.query('UPDATE paymenth2h."BOS_TRANSACTIONS" SET "INTERFACING" = \'1\', "SUCCESS" = \'Y\', "TRXID" = $2 WHERE "REFERENCY" = $1', [referency, resultBody.data.trxId])
...
function getEntryUdoOut(referency, dbName, trxId, outType, error) {
  ...
  fetch(config.base_url_xsjs + "/PaymentService/getEntryOut.xsjs?docnum=" + referency + "&dbName=" + dbName, {
    ...
    .then(data => {
      ...
      Object.keys(data.ENTRY).forEach(async function (key) {
...
function updateUdoTrxId(DocEntry, Lines, trxid, auth, dbName, referency, outType, error) {

  // mendapatakan detail dari pym out
  //jobs.stop();
  let login = LoginV2(dbName);
    ...
    fetch(config.base_url_xsjs + "/PaymentService/get_lines_out.xsjs?Entry=" + DocEntry + "&LineId=" + Lines + "&dbName=" + dbName, {
      ...
      .then(data => {
        ...
        if (login != "") {
          ...
          fetchWithRetry(config.base_url_SL + "/b1s/v2/PYM_OUT(" + parseFloat(DocEntry) + ")", requestOptions, 60000, 3)
            .then(response => {
    ...
    fetch(config.base_url_xsjs + "/PaymentService/get_line_outstatus.xsjs?Entry=" + DocEntry + "&LineId=" + Lines + "&dbName=" + dbName, {
      ...
      .then(data => {
        ...
        if (login != "") {
          ...
          fetchWithRetry(config.base_url_SL + "/b1s/v2/PYMOUT_STATUS(" + parseFloat(DocEntry) + ")", requestOptions, 60000, 3)
            .then(response => {
              if (response.status == 204) {
                logger.info(result)
                executeQuery('UPDATE paymenth2h."BOS_TRANSACTIONS" SET "FLAGUDO" = \'1\' WHERE "REFERENCY" = $1', [referency], async function (error, result, fields) {
